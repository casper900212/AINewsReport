...
def crawl_blocktempo(target_month: str):
    ...
    for page in range(1, 100):
        ...
        if articles:  # 只有在有成功抓取的文章時才寫入檔案
            file_path = f'../../data/blocktempo_{target_month}.json'
            # Create data directory if it doesn't exist
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            try:
                with open(f'../../data/blocktempo_{target_month}.json', 'r+', encoding='utf-8') as f:
                    try:
                        data = json.load(f)
                ...
                with open(f'../../data/blocktempo_{target_month}.json', 'w', encoding='utf-8') as f:
